...
import (
	...
	mockGenre "github.com/go-park-mail-ru/2024_2_NovaCode/internal/genre/mock"
...
func TestUsecase_GetAllByAlbumID_FoundGenres(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	ctx := context.Background()
	genreRepoMock.EXPECT().GetAllByAlbumID(ctx, uint64(1)).Return(genres, nil)

	dtoGenres, err := genreUsecase.GetAllByAlbumID(ctx, uint64(1))
...
func TestUsecase_GetAllByAlbumID_NotFoundGenres(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	ctx := context.Background()
	genreRepoMock.EXPECT().GetAllByAlbumID(ctx, uint64(1)).Return(nil, errors.New("Can't load genres by album ID 1"))

	dtoGenres, err := genreUsecase.GetAllByAlbumID(ctx, uint64(1))

	require.Error(t, err)
	require.Nil(t, dtoGenres)
	require.EqualError(t, err, "Can't load genres by album ID 1")
}
