...
func TestGenreHandlers_GetAllByAlbumIDGenres(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	t.Run("Successful got all genres by album ID", func(t *testing.T) {
		genres := []*dto.GenreDTO{
			{
				ID:      1,
				Name:    "Rock",
		...
		usecaseMock.EXPECT().GetAllByAlbumID(gomock.Any(), uint64(1)).Return(genres, nil)

		router := mux.NewRouter()
		router.HandleFunc("/genres/byAlbumId/{albumId}", genreHandlers.GetAllByAlbumID).Methods("GET")

		request, err := http.NewRequest(http.MethodGet, "/genres/byAlbumId/1", nil)
		assert.NoError(t, err)
	...
	t.Run("Can't find genres by album ID", func(t *testing.T) {
		usecaseMock.EXPECT().GetAllByAlbumID(gomock.Any(), uint64(1)).Return([]*dto.GenreDTO{}, nil)
	...
	t.Run("Invalid album ID", func(t *testing.T) {
		...
		request, err := http.NewRequest(http.MethodGet, "/genres/byAlbumId/abc", nil)
		assert.NoError(t, err)
