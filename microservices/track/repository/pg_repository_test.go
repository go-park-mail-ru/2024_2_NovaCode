...
func TestTrackRepositoryGetPopular(t *testing.T) {
	...
	columns := []string{"id", "name", "duration", "filepath", "image", "artist_id", "album_id", "track_order_in_album", "release_date", "created_at", "updated_at"}
	rows := sqlmock.NewRows(columns)
	for _, track := range tracks {
		rows.AddRow(
			track.ID,
	...
	mock.ExpectQuery(getPopularTracksQuery).WillReturnRows(rows)

	expectedTracks := []*models.Track{&tracks[0], &tracks[1]}
	foundTracks, err := trackRepository.GetPopular(context.Background())
	require.NoError(t, err)
	require.NotNil(t, foundTracks)
	require.Equal(t, foundTracks, expectedTracks)
}
